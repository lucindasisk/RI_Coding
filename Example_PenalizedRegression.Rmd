...
stress_arrange <- stress %>%
  dplyr::select(-c('brain_dv_1':'brain_dv_4'))
...
stress_age_vars <- stress_arrange[2:56,2:32] %>% na.omit()
...
outcome_stress <- stress$brain_dv_1[2:56] %>% na.omit()
...
#See best lambda
best_lambda_test <- (model_stress$lambda[which.min(model_stress$cvm)]) #This saying waht will be the tuning parameter; not important to report for results
